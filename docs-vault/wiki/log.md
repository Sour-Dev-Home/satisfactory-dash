...
> **Frozen (ADR-0033).** Entries after 2026-09-25 live in [`log.d/`](./log.d/README.md), one file per PR,
> so PRs stop conflicting here. Do not edit this file in a PR (CI fails it); `npm run log` prints this
> file followed by the fragments in order, and the coordinator folds them back in with
> `npm run log -- --compact`.
