...
- 2026-09-25 — ADR-0030 PR 2, `platform/secrets/secrets.ts`: AES-256-GCM seal/open for stored game-server tokens. A sealed value is `nonce (12 random bytes) || ciphertext || tag`, stored beside a key id so a rotation keeps old rows readable; an optional `context` (e.g. `${serverId}:api`) is authenticated data, so a value copied to another row or field fails to open. The key comes from `SERVER_SECRETS_KEY` (32 bytes base64, strict; placeholders such as all-zero rejected), with `SERVER_SECRETS_KEY_ID` (default `k1`) and `SERVER_SECRETS_PREVIOUS_KEYS` (`id=base64,...`); unset is allowed until a later PR uses it, set-but-malformed stops the backend at startup, and no error names a key, plaintext or ciphertext. Tamper tests cover the nonce, ciphertext, tag, truncation, wrong key, wrong context and unknown key id.
- 2026-09-25 — C4 model (`workspace.dsl`) refreshed from the architect's validated draft: the Database container and the api -> database relationship are no longer tagged "planned" (PostgreSQL 18, ADR-0025), and DeploymentToday gains a PostgreSQL service node on the gaming PC, the nightly backup task, and an "AWS (backups only)" node with the S3 bucket. Google Sign-In, the Player and the agent stay planned.
...
- 2026-09-25 — ADR-0030 PR 3, schema and repository. Migration `1790467200000_server_connections`: `servers.servers.connection_kind` (`local` default, or `agent`) and `servers.server_connections` (one row per local server: host, `pinned_ip` inet checked to be a single host address, ports 1-65535, `api_token_enc` and optional `frm_token_enc` as sealed bytea with length bounds, one `key_id`, `updated_at`); `LATEST_MIGRATION` moves to it, so a deploy needs `db:migrate`. `modules/servers/repositories/connectionRepository.ts` (raw pg + zod, exported through the servers index): `saveConnection` (upsert, only for a live `local` server), `updateConnection` (row locked; BOTH tokens are opened and sealed again under the current key, so a row never mixes keys), `getConnection` (opened, for the runtime), `getConnectionSummary` (set flags and last 4 characters only, and no suffix for a token under 12 characters), `deleteConnection`, and `listConnections`, which reports rows it cannot open (no keyring, unknown key id, failed open) by key id instead of skipping them, for PR 4's readiness. The AES-GCM context is built in one function from the row's own server id and connection kind plus the field (`${serverId}:${kind}:api|frm`, a tighter form of the architect's `${serverId}:${kind}` so the two columns cannot be swapped). `softDeleteServer` now also deletes the connection row (the encrypted tokens are wiped).
- 2026-09-25 — ADR-0030 PR 4, the runtime and the import. `modules/servers/serverRuntime.ts`: a ServerRuntime that is the ServerDirectory and starts, stops, adds, removes and replaces a server's pollers at runtime (a removed server stops resolving at once; a failing worker never blocks the others). Startup precedence in `server.ts`: with servers stored in the database (`loadDatabaseServers`), the database WINS: the config-built servers are removed and their pollers stopped, the stored ones are built (the pinned address is the host) and started, the operator is made owner of any without one, and one warn names the ignored config variables (names only); with none stored, nothing changes. A stored row that cannot be opened (no key, unknown key id, modified value) is reported, not skipped: readiness is false (503), the backend keeps serving the rest, and one error with `code: SERVER_CONNECTIONS_UNREADABLE` logs each server's public id and key id only. `npm run admin -- import-servers` (one transaction, idempotent, never overwrites, refuses no API token, a stored-TLS setting and more than 8 servers) and `verify-secrets` (opens every stored token, prints counts only; used by the restore rehearsal). `modules/servers/addressGuard.ts`: the ADR address table (loopback and RFC1918 only; IPv4-mapped IPv6 judged by the embedded IPv4; every resolved address must pass; IPv4 pinned before IPv6) used by the import now and by the API routes next. `FrmApiClient` brackets an IPv6 host. New runbook `runbooks/servers.md` (the key and its offline backup, the deploy order with `db:migrate` first, unreadable rows, rotation) and a stored-token check added to the restore rehearsal in `runbooks/backups.md`.
- 2026-09-25 — ADR-0030 PR 4 review fixes: `loadDatabaseServers` re-checks every stored pinned address against the address guard at start and does not serve (or fall back from) a row whose address is not loopback or private (`code: SERVER_CONNECTIONS_REFUSED`, public ids only, readiness false), so a tampered or restored row cannot make the backend send tokens to a public host; `ServerRuntime` settles a worker whose `stop()` throws synchronously (it used to abort a removal halfway).
- 2026-09-25 — ADR-0030 PR 5, the contract and the routes together. Shared: `serverManagement.ts` (create, update and test-connection requests, the write-only `ServerConnection` view, `RESERVED_SERVER_IDS`), five additive error codes (`address_not_allowed`, `connection_test_failed`, `connection_unreadable`, `server_exists`, `server_limit_reached`), `endpoints.serverManagement.{create,testConnection,get,update,remove,testSaved}` each marked `operatorOnly`, and an optional `canManageServers` on the server list; the frontend maps the new codes as unknown for now. Backend: `serverManagement.ts` (the service: resolve every address and pin one, test before saving, cap of 8 under a mutex plus `pg_advisory_xact_lock`, create/update/remove with audit events in the same transaction, the unreadable-row repair path) and `serverManagementRouter.ts` (operator only, per-user rate limits, bodies validated with names never values, mounted as a collection router before the servers router and a scoped router after it). `gameserver/connectionTest.ts` runs QueryServerState and getSessionInfo and returns stable codes only. The generated IDOR tests now honour `operatorOnly` (an owner or admin who is not the operator gets 403; the test stubs the operator as a mere admin to prove it). `ServerRuntime.replace` swaps an entry in one step (no duplicate-id window), gains `rename` and an `onWorkerStopError` hook that logs server id and error name. The runbook `runbooks/servers.md` lists the routes and rules.
- 2026-09-25 — ADR-0030 PR 5 review fixes: the FRM client no longer follows redirects (`redirect: "manual"`; the security review found that fetch would follow a 3xx from a private host to a refused address and send the X-FRM-Authorization header on). The runbook lists the accepted limits (unverified self-signed API certificate on a LAN, the port-probing signal of a test connection, no redirects).
- 2026-09-25 — ADR-0030 PR 5b (architect follow-up to PR 5). Contract: operator-only `GET /api/servers/managed` (`endpoints.serverManagement.list`, `ManagedServerListResponse`), the connection view gains `state: "refused"` (a stored pinned address the address guard rejects), `managed` joins the reserved server ids, and the additive error code `import_required` (409). Backend: the list reads metadata only (`listConnectionMetas`) and opens tokens just to show the last 4 characters, never for a refused row and never without a key (then the row is `unreadable`); `get` reports refused too and works keyless; `testSaved` refuses to connect to a stored address that is not allowed; `remove` no longer needs the keyring; `create` is refused with `import_required` while the environment still configures servers and none is stored (`configuredServerEnvNames` from the gameserver module). The collection router mounts before the servers router, so `managed` is never read as a server id. Runbook updated. Vanilla-API certificate pinning is NOT built (waits for the owner's decision).
- 2026-09-25 — ADR-0030 PR 5b review fixes: the rename-only edit no longer opens the tokens of a refused row (and works without a keyring); `testSaved` checks the stored address before any DNS lookup; the `import_required` gate is re-checked under the advisory lock, and `import-servers` now takes the same lock, so a concurrent import and create cannot slip past each other.
- 2026-09-25 — ADR-0030 amendment 1 added verbatim to `decisions/0030-multiple-servers.md` and implemented as PR 5c, the code gate on LAN servers ("pin before the first LAN server", owner decision). `addressGuard.ts` gains the CONSTANT `LAN_ALLOWED = false` (not an environment variable) and `addressVerdict` (`ok`, `lan`, `refused`): while it is false every path that pins or uses an address also requires loopback. Create, edit, both test connections, `import-servers` and startup (`loadDatabaseServers`, `buildFromConnection`) refuse a LAN address (10/8, 172.16/12, 192.168/16, IPv4-mapped forms), the request paths with the additive error code `lan_requires_cert_pinning` (422) that never names the address; a name that resolves to a mix of loopback and LAN is refused too, public and metadata addresses keep `address_not_allowed`, and a stored LAN row is `refused` in the list, `get` and at start (no server built, so no pollers, no connection, not even a test). Loopback (127/8, ::1, a name resolving only to loopback) is unchanged. The policy is injectable for tests and for the future pinning change only; production never passes one. The pinning itself is NOT built (its design is in the amendment; it waits for the owner wanting a first LAN server). Runbook `runbooks/servers.md` says LAN servers wait for pinning.
- 2026-09-25 — PR 5c review outcome: the fresh-eyes reviews found no way around the LAN gate (every DB-backed path goes through the address verdict with the default policy; nothing in production passes a policy). Recorded in the runbook: servers configured in the environment stay outside the gate (they are the operator's own configuration; `import-servers` refuses a LAN one, so it stays on the environment path until pinning ships). The `address_not_allowed` message no longer says "loopback or private".
- 2026-09-25 — Shared fixtures for server management (ADR-0030), for the frontend's mock server and demo data, with no contract change: `serversOperator` and `serversMemberCannotManage` (the list with `canManageServers` true and false), `serverConnectionOk` and `serverConnectionNoFrmToken`, `managedServersAllStates` (ok, unreadable with no suffixes, refused over the LAN) and `managedServersEmpty`, four `testConnection*` results (passed, API unauthorized, FRM unreachable, API invalid_response), `deleteServerDone`, and error envelopes for operator-only 403, validation 400 (with `detail`), `import_required`, `lan_requires_cert_pinning`, `address_not_allowed`, `connection_test_failed`, `connection_unreadable`, `server_exists` and `server_limit_reached`. Invented values only (fake ids, suffixes like 1a2b, 127.0.0.1 and one RFC 1918 example); each is matched to its shared schema by the fixtures drift test, and a test checks that no envelope names an address or a token.
- 2026-09-25 — ADR-0027 PR 3, history storage. Migration `1790553600000_telemetry_history` (schema `telemetry`: `power_samples`, `item_samples`, `power_rollups`, `item_rollups`, `building_transitions`; additive; `LATEST_MIGRATION` moves to it, so a deploy needs `npm run db:migrate -w backend` first, or the backend will not start). `telemetry/repositories/historyRepository.ts`: hand-written constant SQL by public server id (unnest batches, idempotent minute and hour rollups, batched purges counted through a RETURNING CTE). Raw samples are kept 48 h, 1-minute rollups 30 d, hourly rollups 1 y, transitions 30 d. `BufferedHistoryRecorder` (15 s flush, bounded, never throws, dropped without a database), a hook in `PowerHistoryPoller`, a new 30 s `FactoryHistoryPoller` (item totals summed from `building.production`; state transitions with a baseline reset when over half the ids are new and a per-poll cap), and one process-wide `HistoryMaintenanceWorker` (rollup each minute, 48 h catch-up at start, purge every ~10 min) started with the database workers. Wired through `createTelemetryServices({ history })` for every server, including ones added at runtime. The privacy page now states the retention. New runbook `runbooks/history.md`.
- 2026-09-25 — ADR-0030 hardening (the PR 7 items, done early as one tiny PR): one startup warning, by server id and never the address, when a server configured in the environment is not on loopback ("its API certificate is unverified on the LAN", ADR-0030 amendment 1; `nonLoopbackServerIds` and `isLoopbackHost` in the gameserver config); `DEFAULT_ADDRESS_POLICY` is frozen (a test shows assigning to it throws); and the ids `managed` and `test-connection` (fixed routes under /api/servers) are refused as a server id in the servers file and in `SATISFACTORY_SERVER_ID`, so `import-servers` can never store one.
- 2026-09-25 — ADR-0027 PR 3 review fix (architect): history is NOT recorded while the game is paused, because FRM returns frozen values then (`frm-api.md`; the paused capture reads 3633 MW). `PowerHistoryPoller` skips `recordPower` when `status.isPaused`; `FactoryHistoryPoller` now reads the server status with each snapshot and skips items and transitions while paused, and records nothing when the status cannot be read; `known` is untouched while paused, so a resume makes no fake transitions. The privacy page drops "and send alerts" for now (it returns with the PR that turns alert delivery on); the privacy outline says recording since PR 3; `runbooks/history.md` explains that gaps mean paused or unreachable.
