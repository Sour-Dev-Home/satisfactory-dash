...
- 2026-09-25 — Privacy outline: a row for the in-game names of players connected to a server the reader is a member of (ADR-0029: shown live to that server's members, not stored or logged, from the server owner's own game server), a note that the server owner is responsible for telling their players, and a pre-publication review item [LEGAL] for third-party player names.
