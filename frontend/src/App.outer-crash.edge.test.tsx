...
// Independent (test-hunter) pass: the Log out action in the app-level fallback.
...
describe("App-level fallback Log out", () => {
  it("signs out locally when Log out succeeds from the fallback", async () => {
    let calls = 0;
    server.use(
    ...
    fireEvent.click(within(alert).getByRole("button", { name: "Log out" }));
  ...
  it("with the crash fixed, Try again after Log out shows the sign-in screen", async () => {
    renderWithClient(<App />);
    const alert = await dashboardAlert();
    fireEvent.click(within(alert).getByRole("button", { name: "Log out" }));
    await waitFor(() => expect(within(alert).getByRole("button", { name: "Log out" })).toBeEnabled());
  ...
  it("a crash before sign-in: Log out answered with 401 does not break the fallback", async () => {
    server.use(
  ...
  it("a failed Log out shows its error inside the fallback and Try again still works", async () => {
