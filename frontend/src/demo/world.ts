...
/** A 2-hour stretch with nothing recorded (the game was paused), so every range can show a gap. */
const GAP_FROM_AGO_MS = 30 * 3_600_000;
const GAP_TO_AGO_MS = 28 * 3_600_000;
...
export function historyPower(now: number, range: HistoryRange): HistoryPowerResponse {
  ...
  for (let t = first; t < now; t += step) {
    if (t + step > now - GAP_FROM_AGO_MS && t < now - GAP_TO_AGO_MS) continue;
    starts.push(t);
  }
